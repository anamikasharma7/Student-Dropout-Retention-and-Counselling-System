...
const ExistingStudentForm = () => {
  ...
  const findStudentByUid = () => {
    ...
    fetch(`http://localhost:3000/getStudent?_id=${uidInput}`, requestOptions)
  ...
  const findStudentByAadhar = () => {
    ...
    fetch(
      `http://localhost:3000/getStudent?AadharNumber=${aadharNumber}`,
      requestOptions
    )
      .then((response) => response.json())
      .then((result) => {
  ...
  const addStudentToSchool = () => {
    ...
    fetch(
      `http://localhost:3000/addExistingStudent?schoolID=${sId}&standard=${selectedOption}&studentId=${selectedStudent._id}`,
      requestOptions
    )
