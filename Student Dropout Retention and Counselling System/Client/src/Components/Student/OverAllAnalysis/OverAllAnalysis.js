...
const OverAllAnalysis = () => {
          ...
          if (student.SchoolID && student.SchoolID.length > 0) {
            try {
              ...
              if (marksResult.status === 200) {
                ...
                console.log("[OverAllAnalysis] fetched marks payload", data);
  ...
  // Helper to average array-based marks
  ...
  // Helper function to get grade from marks (accepts number or array)
  const getGrade = (marks) => {
    const avg = getAverage(marks);
    if (avg >= 90) return { grade: "A+", color: "text-green-600", bgColor: "bg-green-100" };
    if (avg >= 80) return { grade: "A", color: "text-green-600", bgColor: "bg-green-100" };
    if (avg >= 70) return { grade: "B+", color: "text-blue-600", bgColor: "bg-blue-100" };
    if (avg >= 60) return { grade: "B", color: "text-blue-600", bgColor: "bg-blue-100" };
    if (avg >= 50) return { grade: "C+", color: "text-yellow-600", bgColor: "bg-yellow-100" };
    if (avg >= 40) return { grade: "C", color: "text-yellow-600", bgColor: "bg-yellow-100" };
  ...
  return (
    <div className="max-w-7xl mx-auto my-6 px-3 sm:px-6">
                    ...
                    <span className="font-semibold">
                      {marksData && marksData.marks ? 
                        (() => {
                          ...
                          const overallAvg = averages.reduce((sum, v) => sum + v, 0) / averages.length;
                    ...
                    <span className="font-semibold">
                      {marksData && marksData.marks ? 
                        (() => {
                          const values = Object.values(marksData.marks);
                          const averages = values.map((v) => getAverage(v));
                          return Math.round(averages.reduce((sum, v) => sum + v, 0) / averages.length);
                        })() : 
                        "N/A"
                      }%
                    </span>
      ...
      {/* Subject Marks Table */}
      {marksData && marksData.marks && (
        <div className="bg-white shadow-lg rounded-xl border border-gray-100 mb-6 sm:mb-8 overflow-x-auto">
                    ...
                    <tr key={subject} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                          avg >= 60 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                        }`}>
                          {avg >= 60 ? "Pass" : "Fail"}
                        </span>
                      </td>
              ...
              <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                marksData && marksData.marks ? 
                  (() => {
                    const values = Object.values(marksData.marks);
                    const averages = values.map((v) => getAverage(v));
                    const average = averages.reduce((sum, v) => sum + v, 0) / averages.length;
