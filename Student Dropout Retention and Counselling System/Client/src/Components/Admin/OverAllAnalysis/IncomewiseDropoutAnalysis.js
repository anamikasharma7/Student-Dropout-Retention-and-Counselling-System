...
const FamilyIncomewiseDropoutAnalysis = ({
  selectedCity,
  selectedTaluka,
  selectedDistrict,
  selectedState,
  ...
  useEffect(() => {
    ...
    fetch(
      `http://localhost:3000/FilterStudentinGroup/FamilyIncome?state=${selectedState}&district=${selectedDistrict}&city=${selectedCity}&taluka=${selectedTaluka}&school`,
      requestOptions
    )
      ...
      .then((result) => {
        ...
        const percentages = datas.StudentsData.map((student, index) => {
          const FamilyIncome = student.FamilyIncome;

          const totalStudent = datas.total.find((total) => total.FamilyIncome === FamilyIncome);
...
export default FamilyIncomewiseDropoutAnalysis;
