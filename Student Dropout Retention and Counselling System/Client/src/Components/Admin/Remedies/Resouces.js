...
const Resources = () => {
    ...
    useEffect(() => {
        const fetchReasons = async () => {
            try {
                const response = await fetch('http://localhost:3000/getReason');
                        ...
                        <div className="mx-5 mt-3">
                            ...
                            {formik.values.links.map((link, index) => (
                                <div key={index} className="flex space-x-2 m-1 ">
                                    ...
                                    />
                                    <button type="button" className="mx-5 bg-gradient-to-r from-orange-500 to-amber-500 text-white font-bold tracking-wider py-1 px-5 rounded-md hover:bg-orange-600 transition-colors duration-300" onClick={() => addLink(index)}>
                                        Add
                                    </button>
                                    {index > 0 && (
                                        <button type="button" className="mx-5 bg-red-700 text-white font-bold tracking-wider py-1 px-5 rounded-md hover:bg-red-600 transition-colors duration-300" onClick={() => removeLink(index)}>
                                            Remove
                                        </button>
                                    )}
                                </div>
